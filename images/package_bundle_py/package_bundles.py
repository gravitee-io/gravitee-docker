...
def prepare_gateway_bundle(gateway):
    ...
    ignored_packages = [".*gravitee-repository-elasticsearch.*", ".*gravitee-apim-repository-hazelcast.*"]
    # if version < 3.15.0, also ignore redis-repository
    if int(version_param.split(".")[1]) < 15:
        ignored_packages.append(".*gravitee-apim-repository-redis.*")

    copy_files_into(repositories_path, bundle_path + "plugins", ignored_packages)
...
def prepare_mgmt_bundle(mgmt):
    ...
    ignored_packages = [".*gravitee-repository-ehcache.*", ".*gravitee-apim-repository-gateway-bridge-http-client.*", ".*gravitee-apim-repository-gateway-bridge-http-server.*", ".*gravitee-apim-repository-hazelcast.*"]
    # if version < 3.15.0, also ignore redis-repository
    if int(version_param.split(".")[1]) < 15:
        ignored_packages.append(".*gravitee-apim-repository-redis.*")
